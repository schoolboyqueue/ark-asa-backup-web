...
import { BACKUP_STORAGE_DIRECTORY, ARK_SAVE_DIRECTORY, ARK_SERVER_CONTAINER_NAME } from '../config/constants.js';
 ...
 */
sseRouter.get('/api/server/status/stream', async (_httpRequest: Request, httpResponse: Response) => {
 ...
 */
sseRouter.get('/api/backup/health/stream', async (_httpRequest: Request, httpResponse: Response) => {
  const sendEvent = initializeSSEStream(httpResponse);
