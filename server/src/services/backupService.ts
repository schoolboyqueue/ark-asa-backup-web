...
export async function saveVerificationResult(
  ...
  const verificationFilePath = path.join(BACKUP_STORAGE_DIRECTORY, `${backupFileName}.verify.json`);
 ...
 * @async
 */
export async function loadVerificationResult(
  backupFileName: string
      ...
      for (const oldBackup of backupFilesWithMetadata.slice(maximumBackupsToRetain)) {
        try {
          ...
          const metadataFilePath = path.join(
            BACKUP_STORAGE_DIRECTORY,
            `${oldBackup.name}.meta.json`
          );
          const verificationFilePath = path.join(
            BACKUP_STORAGE_DIRECTORY,
            `${oldBackup.name}.verify.json`
          );
